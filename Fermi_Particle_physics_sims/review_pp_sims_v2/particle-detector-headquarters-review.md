...
**PHY-P2-2 update (2026-08-26, later same day):** user chose the caption option — the HCAL meter now renders a muted "ECAL + HCAL = kinetic energy" caption under its GeV value for hadron events only (first shipped as "kinetic energy deposited"; reworded same day at the user's request — the original phrasing implied the HCAL value alone equals KE, when KE is the two meters' sum) (`meter()` gained an optional `cap` argument; passed as `ev.P.kind === 'had' ? 'kinetic energy deposited' : null`). Deposit numbers unchanged. Verified headlessly via fillText capture: caption present for π⁺ and p, absent for e⁻ and μ⁻; deposits byte-identical (π⁺ 0.26+0.61, p 0.13+0.30, e⁻ 1.00, μ 0.10+0.20); screenshot `dhq-cap-pion2.png` shows clean rendering; zero pageerrors. **CLOSED.**
